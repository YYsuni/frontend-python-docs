...
pprint({"key": {"nested": [{"id": i} for i in range(3)]}})
```
...
print(sql_like.safe_substitute(col="name", id="42"))  # 缺键不炸，原样保留
```
...
cache["k"] = obj
del obj
print(list(cache.keys()))  # 可能没有 'k' —— 随 GC 时机变化
```
